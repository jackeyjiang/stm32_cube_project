/*******************************************************************************
 * FreeRTOS+Trace v2.7.0 Demo Application
...
int vStartDemoApplication(void)
{
	...
	xTaskCreate( trcDemoTaskSensor, "SensorX",
				configMINIMAL_STACK_SIZE*4, &sensorTaskID1, 8, &objectHandle );
	...
	xTaskCreate( trcDemoTaskSensor, "SensorY",
				configMINIMAL_STACK_SIZE*4, &sensorTaskID2, 7, NULL );
	if (objectHandle == NULL)
	{
		return -7;
	}
	...
	xTaskCreate( trcDemoTaskSensor, "SensorZ",
				configMINIMAL_STACK_SIZE*4, &sensorTaskID3, 6, NULL );
	if (objectHandle == NULL)
	{
		return -8;
	}
	...
	xTaskCreate( trcDemoTaskController, "Control",
				configMINIMAL_STACK_SIZE*4, NULL, 4, &objectHandle );
	if (objectHandle == NULL)
	{
		return -9;
	}
	...
	xTaskCreate( trcDemoTaskActuator, "Actuator",
				configMINIMAL_STACK_SIZE*4, NULL, 5, &objectHandle );
	if (objectHandle == NULL)
	{
		return -10;
	}
